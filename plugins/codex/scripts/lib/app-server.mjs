  ...
  static async connect(cwd, options = {}) {
    ...
    if (!options.disableBroker) {
      ...
      if (!brokerEndpoint && options.reuseExistingBroker) {
        // A recorded broker session can be stale — a previous Claude
        // session's broker that has since exited. Only reuse it if
        // something is actually listening; otherwise fall through to a
        // directly spawned app-server instead of failing on a dead pipe.
        const recorded = loadBrokerSession(cwd)?.endpoint ?? null;
        brokerEndpoint = recorded && (await waitForBrokerEndpoint(recorded, 150)) ? recorded : null;
      }
      if (!brokerEndpoint && !options.reuseExistingBroker) {
